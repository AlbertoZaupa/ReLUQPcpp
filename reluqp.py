...
class ReLU_QP(object):
    ...
    def compute_residuals_OSQP(H, A, M, g, x, z, z_prev, lam, rho, rho_min: float, rho_max: float):
        ...
        dual_res = torch.linalg.norm(rho * A.T @ M @ (z_prev - z), ord=np.inf)
        lagrangian_grad = torch.linalg.vector_norm(t2 + t3 + g, ord=torch.inf)
        numerator = torch.div(primal_res, torch.max(torch.linalg.vector_norm(t1, ord=torch.inf),
                                                    torch.linalg.vector_norm(z, ord=torch.inf)))
        denom = torch.div(lagrangian_grad, torch.max(
